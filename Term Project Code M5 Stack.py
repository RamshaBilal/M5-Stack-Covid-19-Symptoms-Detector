...
class Question:
  def __init__(self, question, A="Yes", B="Maybe", C="No", answer = "No"):
  ...
  while True:
    ...
    for question in questions:
      if question.getAnswer() == "Yes":
        counter = counter + 1
      elif question.getAnswer() == "Maybe":
        counter = counter + 0.5
      
    if counter > 3:
      screen.set_screen_bg_color(0xFF0000)
      errorMessage = M5Label("COVID SUSPICIOUS", color=0xFFFFFF, font=FONT_MONT_26, parent=None)
      errorMessage.set_align(ALIGN_CENTER, x=0, y=0, ref=screen.obj)
    else:
      screen.set_screen_bg_color(0x00AA00)
      errorMessage = M5Label("COVID NOT DETECTED", color=0xFFFFF, font=FONT_MONT_26, parent=None)
      errorMessage.set_align(ALIGN_CENTER, x=0, y=0, ref=screen.obj)
